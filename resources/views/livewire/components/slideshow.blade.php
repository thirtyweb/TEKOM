<div>
    <div class="relative w-full h-[75vh] max-h-[700px] min-h-[520px] overflow-hidden bg-black p-2 md:p-3 border-2 border-green-500/20"
     x-data="{
         currentSlide: 0,
        ...
        <div class="relative w-full h-full">
            @foreach($allSlides as $index => $slide)
                <div x-show="currentSlide === {{ $index }}" 
                    ...
                    <div class="absolute inset-0 flex flex-col items-center justify-center text-center p-6 md:p-10">
                        <div class="text-white max-w-4xl transform transition-all duration-1000 ease-out"
                                ...
                                <div>
                                    <a wire:navigate href="{{ $slide['link_url'] }}" 
                                       class="inline-flex items-center bg-green-500 text-black font-bold py-3 px-8 rounded-none hover:bg-green-400 transition-colors duration-300 group relative overflow-hidden">
                                        ...
                                        <span class="absolute inset-0 bg-green-700 opacity-0 group-hover:opacity-100 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left"></span>
                                    </a>
                                </div>
                            @endif
                        </div>
...
<style>
    /* Keyframes for the image pan/zoom effect */
    @keyframes pan-zoom {
        0% {
    ...
    .animate-pan-zoom {
        animation: pan-zoom 15s infinite ease-in-out alternate;
    }

    /* Keyframes for the glitch effect on the title */
    @keyframes glitch-effect {
        ...
        2% {
            ...
            text-shadow: 2px 2px 0 red, -2px -2px 0 cyan, 0 0 15px rgba(52,211,153,0.7);
        }
        4% {
            transform: translate(2px, 2px);
            text-shadow: -2px -2px 0 red, 2px 2px 0 cyan, 0 0 15px rgba(52,211,153,0.7);
        ...
        6% {
            ...
            text-shadow: 1px -1px 0 red, -1px 1px 0 cyan, 0 0 15px rgba(52,211,153,0.7);
        }
        8% {
            transform: translate(1px, -1px);
            text-shadow: -1px 1px 0 red, 1px -1px 0 cyan, 0 0 15px rgba(52,211,153,0.7);
    ...
    .animate-glitch {
        animation: glitch-effect 7s infinite linear forwards; /* Adjust duration as needed */
    }

    /* General pulse effect, already there but for reference */
    @keyframes pulse {
        0%, 100% {
            opacity: 1;
        }
